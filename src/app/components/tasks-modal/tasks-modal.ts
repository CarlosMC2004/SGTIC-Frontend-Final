// src/app/components/tasks-modal/tasks-modal.ts
...
interface WeekDayOption { label: string; value: string; }
interface TaskFormModel { name: string; backupType: BackupType; frequency: FrequencyType; dayOfWeek: string; executionTime: string; active: boolean; }
...
export class TasksModal implements OnInit {
  ...
  weekDays: WeekDayOption[] = [
    { label: 'L', value: 'MONDAY' }, { label: 'M', value: 'TUESDAY' },
    { label: 'X', value: 'WEDNESDAY' }, { label: 'J', value: 'THURSDAY' },
    { label: 'V', value: 'FRIDAY' }, { label: 'S', value: 'SATURDAY' },
    { label: 'D', value: 'SUNDAY' }
  ];

  taskForm: TaskFormModel = {
    name: '', backupType: 'FULL', frequency: 'DAILY',
    dayOfWeek: 'MONDAY', executionTime: '02:00', active: true
  };
  ...
  toggleNewTask(): void {
    ...
    this.errorMessage = '';
    this.successMessage = '';
    if (!this.isNewTaskPanelOpen) this.resetForm();
    this.cdr.detectChanges();
  }

  selectFrequency(frequency: FrequencyType): void {
    this.taskForm.frequency = frequency;
    if (frequency === 'DAILY') this.taskForm.dayOfWeek = 'MONDAY';
    this.cdr.detectChanges();
  }
  ...
  saveTask(): void {
    ...
    if (!this.activeConfigId) { this.errorMessage = 'Configure primero el respaldo.'; return; }
    if (!this.isValidForm()) return;

    this.isSaving = true;
    this.cdr.detectChanges(); // UI a Guardando...

    const payload: BackupScheduleRequest = {
      backupConfigId: this.activeConfigId, userId,
      name: this.taskForm.name.trim(), backupType: this.taskForm.backupType,
      frequency: this.taskForm.frequency, dayOfWeek: this.taskForm.frequency === 'WEEKLY' ? this.taskForm.dayOfWeek : null,
      executionTime: this.normalizeTimeForRequest(this.taskForm.executionTime), active: this.taskForm.active
    };
  ...
  toggleTaskStatus(task: BackupScheduleResponse): void {
    ...
    const payload: BackupScheduleRequest = {
      backupConfigId: task.backupConfigId, userId, name: task.name,
      backupType: task.backupType as BackupType, frequency: task.frequency as FrequencyType,
      dayOfWeek: task.frequency === 'WEEKLY' ? task.dayOfWeek : null,
      executionTime: this.normalizeTimeForRequest(task.executionTime), active: !task.active
    };
  ...
  getFrequencyLabel(frequency: string): string { return frequency === 'DAILY' ? 'Diaria' : 'Semanal'; }

  getScheduleLabel(task: BackupScheduleResponse): string {
    const time = this.formatTime(task.executionTime);
    if (task.frequency === 'DAILY') return `Todos los días / ${time}`;
    return `${this.getDayLabel(task.dayOfWeek)} / ${time}`;
  }

  getStatusLabel(task: BackupScheduleResponse): string { return task.active ? 'Activo' : 'Inactivo'; }
  trackByTask(_: number, task: BackupScheduleResponse): number { return task.id; }
  ...
  private resetForm(): void {
    this.taskForm = { name: '', backupType: 'FULL', frequency: 'DAILY', dayOfWeek: 'MONDAY', executionTime: '02:00', active: true };
  }

  private isValidForm(): boolean {
    if (!this.taskForm.name.trim()) { this.errorMessage = 'El nombre de la tarea es obligatorio.'; return false; }
    if (!this.taskForm.executionTime.trim()) { this.errorMessage = 'La hora de ejecución es obligatoria.'; return false; }
    if (this.taskForm.frequency === 'WEEKLY' && !this.taskForm.dayOfWeek) { this.errorMessage = 'Debe seleccionar un día para la frecuencia semanal.'; return false; }
    return true;
  }
