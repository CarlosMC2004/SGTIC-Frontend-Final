...
export class DriveModal implements OnInit, OnDestroy {
  ...
  unlinkGoogleDrive(): void {
    if (!this.activeAccount?.id || this.isUnlinking) {
      return;
    }

    const confirmed = window.confirm(
      `¿Desea desvincular la cuenta ${this.activeAccount.googleEmail || 'de Google Drive'}?`
    );
  ...
  private openOAuthWindow(authUrl: string): void {
    const popup = window.open(
      authUrl,
      'google-drive-oauth',
      'width=640,height=760,menubar=no,toolbar=no,location=yes,status=no'
    );
